...
- **NRR tracking**: live, 76 `nrr_snapshots` rows for `2026-07-08` using the same universe definition as MRR (`Closing_Date` in the target month AND `Probability >= 70`), tracking Zoho's raw `NRR_Amount` ("Expected NRR") field, not `Adjusted_NRR` (which discounts it by Probability). The `Probability >= 70` filter gates which deals qualify for the universe at all; it doesn't scale the NRR dollar figure. A deal can be MRR-only, NRR-only, or both; `deals` covers the union of both universes
...
- `nrr_snapshots` - same shape as `mrr_snapshots` but `nrr_amount`, populated only for deals with nonzero `NRR_Amount` (Expected NRR, unweighted by Probability). This is what the dashboard's NRR metric tab reads; it is the "new closures NRR" view, using the same Closing_Date + Probability >= 70 universe as MRR, not a Stage=Closed Won filter
